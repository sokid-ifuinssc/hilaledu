...
class TendikImport implements ToCollection, WithHeadingRow, SkipsEmptyRows
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $index => $row) {
            ...
            // Ekstrak NUPTK / NIP (kolom wajib mutlak bagi tendik)
            $nuptk = trim((string)($row['nuptk'] ?? $row['nip'] ?? $row['no_nuptk'] ?? $row['id_tendik'] ?? ''));
            ...
            if (empty($nuptk)) {
                $this->errors[] = [
                    ...
                    'column'     => 'NUPTK',
                    'identifier' => '-',
                    'message'    => 'Kolom NUPTK kosong. NUPTK wajib diisi sebagai identitas unik tenaga kependidikan.',
                ];
                continue;
            }

            // Ekstrak Nama Lengkap (jika kosong, fallback otomatis)
            $name = trim((string)($row['nama_lengkap'] ?? $row['nama'] ?? $row['nama_tendik'] ?? ''));
            if (empty($name)) {
                $name = 'Tendik ' . $nuptk;
            }
            ...
            if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $cleanNuptk = preg_replace('/[^a-zA-Z0-9]/', '', $nuptk);
                $email = $cleanNuptk . '@tendik.hilaledu.sch.id';
            }
            ...
            // Cek kesamaan data (apakah tendik ini sudah ada di database)
            $existing = User::where('role', 'tendik')
    ...
    protected function ensureUniqueEmail(string $email): string
    {
        ...
        while (User::where('email', $candidate)->exists()) {
            $parts = explode('@', $email);
            $candidate = $parts[0] . '_' . $counter++ . '@' . ($parts[1] ?? 'tendik.hilaledu.sch.id');
        }
        return $candidate;
    }
