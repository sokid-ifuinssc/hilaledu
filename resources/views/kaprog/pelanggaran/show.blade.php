...
<div class="space-y-6 max-w-4xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-4"><h3 class="text-lg font-semibold text-gray-800">Detail Pelanggaran</h3><span class="badge text-sm {{ $pelanggaran->status_badge }}">{{ $pelanggaran->status_label }}</span></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <a href="{{ route('kaprog.progres.show', $pelanggaran) }}" class="btn-primary text-sm">📋 Lihat Detail Progres →</a>
    </div>
    @endif

    <div class="flex gap-3">
        <a href="{{ route('kaprog.pelanggaran.index') }}" class="btn-secondary">← Kembali</a>
        @if($progres)
        <a href="{{ route('kaprog.progres.show', $pelanggaran) }}" class="btn-primary">📋 Detail Progres</a>
        @endif
    </div>
</div>
@endsection
...
<div class="help-section">
    ...
    <p>Informasi lengkap pelanggaran siswa. Gunakan menu Progres Data Pelanggaran untuk approve atau isi laporan.</p>
</div>
@endsection
