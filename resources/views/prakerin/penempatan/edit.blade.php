...
@section('title', 'Edit Penempatan Prakerin')
@section('page-title', 'Edit Penempatan Prakerin')
        ...
        <div class="col-lg-9">
            <div class="card border-0 shadow-sm" style="background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); border-radius: 16px;">
                <div class="card-header bg-transparent border-secondary p-4 pb-0">
                    <h5 class="fw-bold text-white mb-1"><i class="bi bi-pencil-square text-warning me-2"></i>Edit Penempatan Siswa Prakerin</h5>
                    <p class="text-white-50 small">Perbarui data lokasi penempatan atau guru dan pembimbing DU/DI.</p>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('prakerin.penempatan.update', $penempatan->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="periode_prakerin_id" id="periode_prakerin_id" class="form-select bg-dark text-white border-secondary @error('periode_prakerin_id') is-invalid @enderror" required>
                                    @foreach($periodeList as $p)
                                        <option value="{{ $p->id }}" {{ old('periode_prakerin_id', $penempatan->periode_prakerin_id) == $p->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="siswa_id" class="form-label text-white">Siswa <span class="text-danger">*</span></label>
                                <select name="siswa_id" id="siswa_id" class="form-select bg-dark text-white border-secondary @error('siswa_id') is-invalid @enderror" required>
                                    @foreach($siswaList as $s)
                                        <option value="{{ $s->id }}" {{ old('siswa_id', $penempatan->siswa_id) == $s->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="dudi_id" id="dudi_id" class="form-select bg-dark text-white border-secondary @error('dudi_id') is-invalid @enderror" required>
                                    @foreach($dudiList as $d)
                                        <option value="{{ $d->id }}" {{ old('dudi_id', $penempatan->dudi_id) == $d->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="pembimbing_dudi_id" id="pembimbing_dudi_id" class="form-select bg-dark text-white border-secondary @error('pembimbing_dudi_id') is-invalid @enderror" required>
                                    @foreach($pembimbingList as $pb)
                                        <option value="{{ $pb->id }}" {{ old('pembimbing_dudi_id', $penempatan->pembimbing_dudi_id) == $pb->id ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="guru_id" id="guru_id" class="form-select bg-dark text-white border-secondary @error('guru_id') is-invalid @enderror" required>
                                    @foreach($guruList as $g)
                                        <option value="{{ $g->id }}" {{ old('guru_id', $penempatan->guru_id) == $g->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status" id="status" class="form-select bg-dark text-white border-secondary @error('status') is-invalid @enderror" required>
                                    <option value="belum_mulai" {{ old('status', $penempatan->status) == 'belum_mulai' ? 'selected' : '' }}>Belum Mulai</option>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="tanggal_mulai" id="tanggal_mulai" class="form-control bg-dark text-white border-secondary @error('tanggal_mulai') is-invalid @enderror" value="{{ old('tanggal_mulai', $penempatan->tanggal_mulai?->format('Y-m-d')) }}" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="tanggal_selesai" id="tanggal_selesai" class="form-control bg-dark text-white border-secondary @error('tanggal_selesai') is-invalid @enderror" value="{{ old('tanggal_selesai', $penempatan->tanggal_selesai?->format('Y-m-d')) }}" required>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="keterangan" id="keterangan" class="form-control bg-dark text-white border-secondary" rows="2">{{ old('keterangan', $penempatan->keterangan) }}</textarea>
                        </div>

                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary px-4"><i class="bi bi-save me-1"></i> Perbarui Penempatan</button>
