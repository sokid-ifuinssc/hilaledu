...
@section('subtitle', $roleKey === 'waka' ? 'Pelanggaran yang membutuhkan persetujuan Waka Kesiswaan' : 'Pelanggaran yang membutuhkan persetujuan Kepala Sekolah')
...
<div class="space-y-6">
    ...
    {{-- Filters --}}
    @php $routePrefix = $roleKey === 'waka' ? 'waka' : 'kepsek'; @endphp
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            ...
            <a href="{{ route($routePrefix . '.progres.index') }}" class="btn-secondary text-sm">Reset</a>
        </form>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                @forelse($progresList as $progres)
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="py-3 px-4">
                            @if($progres->requiresApprovalFrom($roleKey) && $progres->$approvalField === 'belum')
                                <span class="badge text-xs bg-yellow-100 text-yellow-800">⏳ Perlu Approve</span>
                            @elseif($progres->requiresLaporanFrom($roleKey) && empty($progres->$laporanField))
                        ...
                        <td class="py-3 px-4"><a href="{{ route($routePrefix . '.progres.show', $p) }}" class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">Detail →</a></td>
                    </tr>
                @empty
