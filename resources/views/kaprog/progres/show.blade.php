...
<div class="space-y-6 max-w-4xl">
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1 pb-4 border-b border-gray-100">
                    ...
                    <div class="mt-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                        ...
                        <form method="POST" action="{{ route('kaprog.progres.approve', $pelanggaran) }}" enctype="multipart/form-data">
                            @csrf
                            <div class="space-y-3">
                                <div>
                                    <label class="form-label text-sm">Catatan (opsional)</label>
                                    <textarea name="catatan_kaprog" rows="2" class="form-textarea text-sm w-full" placeholder="Tambahkan catatan persetujuan..."></textarea>
                                </div>
                                <div>
                                    <label class="form-label text-sm">Dokumen Pendukung (opsional)</label>
                                    <input type="file" name="dokumen_kaprog" class="form-input text-sm">
                                </div>
                                <div class="flex gap-2">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1 pb-4 border-b border-gray-100">
                    ...
                    {{-- Form Laporan Kaprog --}}
                    @if($progres->requiresLaporanFrom('kaprog') && empty($progres->laporan_kaprog))
                        @php
                            $canShowLaporanForm = $progres->approval_kaprog === 'disetujui' && (!$progres->requiresCetakSurat() || $progres->surat_dicetak);
                        @endphp
                        @if($canShowLaporanForm)
                        <div class="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                            <h4 class="text-sm font-semibold text-purple-800 mb-2">📝 Isi Laporan Kaprog</h4>
                            <p class="text-xs text-purple-600 mb-3">Tuliskan laporan hasil penyampaian {{ $progres->jenis_tindakan_label }}</p>
                            <form method="POST" action="{{ route('kaprog.progres.laporan', $pelanggaran) }}" enctype="multipart/form-data">
                                @csrf
                                <textarea name="laporan_kaprog" rows="3" class="form-textarea text-sm w-full mb-2" placeholder="Tulis laporan..." required></textarea>
                                <input type="file" name="dokumen_laporan_kaprog" class="form-input text-sm mb-2">
                        ...
                        @endif
                    @elseif(!empty($progres->laporan_kaprog))
                    <div class="mt-3 p-3 bg-green-50 rounded-lg">
                        <p class="text-xs font-semibold text-green-800">✅ Laporan Anda sudah terisi</p>
                        <p class="text-xs text-green-700">{{ $progres->laporan_kaprog }}</p>
                    </div>
                    @endif
                </div>
            </div>
    ...
    <div class="flex gap-3">
        <a href="{{ route('kaprog.progres.index') }}" class="btn-secondary">← Kembali ke Progres</a>
        <a href="{{ route('kaprog.pelanggaran.show', $pelanggaran) }}" class="btn-secondary">📋 Detail Pelanggaran</a>
    </div>
</div>
@endsection
